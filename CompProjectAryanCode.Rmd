...
# A milli 
url <- "https://www.shazam.com/song/1508562516/heat-waves"
...
```{r}
write.csv(longest_lyrics_df,"Data.csv", row.names = TRUE)
```
...
x <- unnest_tokens(longest_lyrics_df, word, lyrics)
```
...
```{r}
predict(model, c("yeah"), type = "nearest", top_n = 10)
```
